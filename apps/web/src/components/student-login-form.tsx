...
export function StudentLoginForm() {
  ...
  async function requestCode(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const result: { message?: string; developmentCode?: string } = await response.json();
      ...
      const developmentHint = result.developmentCode
        ...
        : "";
      setMessage(`${result.message ?? "Enter the code to continue."}${developmentHint}`);
      setStep("otp");
    } catch {
    ...
    <section className="w-full max-w-md justify-self-end rounded-3xl border border-[#d9e5df] bg-white p-6 shadow-[0_24px_60px_-35px_rgba(20,73,62,0.45)] sm:p-8">
      ...
      {step === "matric" ? (
        ...
        <form onSubmit={verifyCode} className="space-y-5">
          <div>
            <label htmlFor="otp-code" className="mb-2 block text-sm font-medium text-[#294946]">Six-digit verification code</label>
            <input
              id="otp-code"
              name="code"
              type="text"
              ...
              required
              autoFocus
              className="h-[3.25rem] w-full rounded-xl border border-[#c9d9d2] bg-[#fbfdfc] px-4 text-center font-mono text-lg tracking-[0.35em] text-[#17312f] outline-none transition placeholder:tracking-[0.35em] placeholder:text-[#91a6a0] focus:border-[#16806b] focus:ring-4 focus:ring-[#dff3eb]"
            />
            <p className="mt-2 text-xs leading-5 text-[#708a84]">The code expires in 10 minutes and can be used once.</p>
          </div>
          <button type="submit" disabled={isSubmitting || code.length !== 6} className="flex h-[3.25rem] w-full items-center justify-center rounded-xl bg-[#0e5a4f] px-4 text-sm font-semibold text-white transition hover:bg-[#0a493f] focus:outline-none focus:ring-4 focus:ring-[#b8ded1] disabled:cursor-not-allowed disabled:opacity-70">
            {isSubmitting ? "Verifying..." : "Verify and continue"}
          </button>
          <button type="button" onClick={() => { setStep("matric"); setCode(""); setMessage(undefined); }} className="w-full text-sm font-medium text-[#315954] underline-offset-4 hover:text-[#0e5a4f] hover:underline">
            Use a different matric number
          </button>
        </form>
      )}

      {message && <p className="mt-5 rounded-xl bg-[#edf8f3] px-3 py-2.5 text-sm leading-5 text-[#176353]" role="status">{message}</p>}
